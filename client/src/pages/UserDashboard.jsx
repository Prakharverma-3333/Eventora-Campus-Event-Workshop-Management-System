                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {bookings.map((booking) => (
                        <div key={booking._id} className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md transition border border-gray-100 flex flex-col">
                            ...
                            <div className="p-4 bg-gray-50 flex justify-end items-center shrink-0">
                                {booking.eventId && booking.status !== 'cancelled' ? (
                                    <button
                                        onClick={() => cancelBooking(booking._id)}
                                        className="text-red-600 hover:text-red-700 font-bold text-sm bg-red-50 hover:bg-red-100 px-3.5 py-1.5 rounded-lg border border-red-200 transition flex items-center gap-1.5 cursor-pointer shadow-xs"
                                    >
                                        <FaTimesCircle className="text-xs" /> Cancel Booking
                                    </button>
                                ) : (
                                    <div className="w-full text-center text-sm text-gray-400 font-medium italic">Booking Cancelled</div>
                                )}
                            </div>
                        </div>
                    ))}
