        ...
        <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
                    <div>
                        <p className="text-gray-500 text-xs font-bold uppercase tracking-wider mb-1">Paid Bookings</p>
                        <h3 className="text-2xl font-black text-indigo-600">{bookings.filter(b => b.status === 'confirmed' && b.amount > 0).length}</h3>
                    </div>
                    <div className="w-11 h-11 bg-indigo-50 text-indigo-600 rounded-xl flex items-center justify-center text-lg font-bold">💳</div>
                </div>

                <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
                    <div>
                        <p className="text-gray-500 text-xs font-bold uppercase tracking-wider mb-1">Free Bookings</p>
                        <h3 className="text-2xl font-black text-blue-600">{bookings.filter(b => b.status === 'confirmed' && b.amount === 0).length}</h3>
                    </div>
                    <div className="w-11 h-11 bg-blue-50 text-blue-600 rounded-xl flex items-center justify-center text-lg font-bold">🎟️</div>
                </div>

                <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
                    <div>
                        <p className="text-gray-500 text-xs font-bold uppercase tracking-wider mb-1">Pending Requests</p>
                        <h3 className="text-2xl font-black text-amber-600">{bookings.filter(b => b.status === 'pending').length}</h3>
                    </div>
                    <div className="w-11 h-11 bg-amber-50 text-amber-600 rounded-xl flex items-center justify-center text-lg font-bold">⏳</div>
                </div>

                <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
                    <div>
                        <p className="text-gray-500 text-xs font-bold uppercase tracking-wider mb-1">Total Revenue</p>
                        <h3 className="text-2xl font-black text-green-600">₹{bookings.reduce((sum, b) => b.status === 'confirmed' ? sum + b.amount : sum, 0)}</h3>
                    </div>
                    <div className="w-11 h-11 bg-green-50 text-green-600 rounded-xl flex items-center justify-center text-lg font-bold">₹</div>
                </div>
            </div>
            ...
            {showEventForm && (
                <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 mb-8 animation-slideDown">
                    ...
                    <form onSubmit={handleCreateEvent} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <select required className="border px-4 py-3 rounded-lg focus:ring-2 focus:ring-gray-700 outline-none transition bg-white" value={formData.category} onChange={e => setFormData({ ...formData, category: e.target.value })}>
                        ...
                        <select
                            required
                            className="border px-4 py-3 rounded-lg focus:ring-2 focus:ring-gray-700 outline-none transition bg-white"
                            value={formData.location}
                            onChange={e => setFormData({ ...formData, location: e.target.value })}
                        >
                            <option value="">Select Location</option>
                            <option value="Campus Tech Auditorium, Block B">Campus Tech Auditorium, Block B</option>
                            <option value="University Open Air Amphitheatre">University Open Air Amphitheatre</option>
                            <option value="Student Activity Center, Gaming Lounge">Student Activity Center, Gaming Lounge</option>
                            <option value="Engineering Block Hall 4">Engineering Block Hall 4</option>
                            <option value="University Sports Complex & Stadium">University Sports Complex & Stadium</option>
                            <option value="Amphitheatre Lawn">Amphitheatre Lawn</option>
                            <option value="Student Activity Main Hall">Student Activity Main Hall</option>
                            <option value="Seminar Hall B, IT Block">Seminar Hall B, IT Block</option>
                            <option value="University Main Auditorium">University Main Auditorium</option>
                            <option value="Conference Room 1">Conference Room 1</option>
                            <option value="Central Plaza Stage">Central Plaza Stage</option>
                            <option value="Computer Science Lab 3 & 4">Computer Science Lab 3 & 4</option>
                            <option value="Tech Innovation Hub, Room 102">Tech Innovation Hub, Room 102</option>
                            <option value="Main Campus Ground">Main Campus Ground</option>
                        </select>
                                    ...
                                    <li key={booking._id} className={`p-6 hover:bg-gray-50 transition border-l-4 ${booking.status === 'pending' ? 'border-l-yellow-400' : booking.status === 'confirmed' ? 'border-l-green-400' : 'border-l-red-400'}`}>
                                        ...
                                        {booking.status === 'pending' && (
                                            <div className="flex flex-wrap gap-2 mt-2">
                                                {booking.amount > 0 ? (
                                                    <button onClick={() => handleConfirmBooking(booking._id, 'paid')} className="flex-1 min-w-[120px] bg-green-50 text-green-700 hover:bg-green-600 hover:text-white border border-green-200 text-xs font-bold py-2.5 px-3 rounded-lg shadow-sm transition cursor-pointer">
                                                        ✓ Approve as Paid
                                                    </button>
                                                ) : (
                                                    <button onClick={() => handleConfirmBooking(booking._id, 'paid')} className="flex-1 min-w-[120px] bg-blue-50 text-blue-700 hover:bg-blue-600 hover:text-white border border-blue-200 text-xs font-bold py-2.5 px-3 rounded-lg shadow-sm transition cursor-pointer">
                                                        ✓ Confirm Seat
                                                    </button>
                                                )}
                                                <button onClick={() => handleCancelBooking(booking._id)} className="w-[80px] bg-red-50 text-red-600 hover:bg-red-500 hover:text-white border border-red-200 text-xs font-bold py-2.5 px-3 rounded-lg transition cursor-pointer">
                                                    ✕ Reject
                                                </button>
                                            </div>
                                        )}
